...
public class WorkService {
    ...
    public List<Work> getUnfinishedStudentByTaskId(Work wk) {
        List<Work> works = new ArrayList<Work>();
        Connection conn = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        String sql = "SELECT studentcourse.studentId,`user`.userName FROM studentcourse " +
                "LEFT JOIN `user` ON `user`.userId = studentcourse.studentId " +
                "LEFT JOIN (SELECT DISTINCT(studentId) FROM `work` WHERE `work`.taskId= ?) AS studentwork " +
                "ON studentcourse.studentId = studentwork.studentId " +
                "WHERE  `studentcourse`.teacherCourseId = ? AND studentwork.studentId IS NULL";
        try {
            conn = DbUtil.getConnection();
            ps = conn.prepareStatement(sql);
            ps.setInt(1,wk.getTaskId());
            ps.setInt(2,wk.getTeacherCourseId());
            rs = ps.executeQuery();
            while (rs.next()) {
                Work work= new Work();
                work.setUserId(rs.getString("studentId"));
