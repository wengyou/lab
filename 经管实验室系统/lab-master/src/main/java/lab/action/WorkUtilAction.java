...
//		String s = new String(urls.getBytes("iso-8859-1"), "GBk");
		System.out.println(urls+"------------ file address");
		// 文件地址分割
		//StringTokenizer st = new StringTokenizer(urls, ""); // 可以更换分隔符为>,
		String[] url=urls.split("(>,)|(>)");//以>,或者>为分隔符
		...
		//while (st.hasMoreTokens()) {
			...
			File path = new File(url[i].replace("\\\\", "\\\\\\\\"));
			if (path.isFile()) {
				file = new File(url[i]);
				al.add(file);
			}
